  ...
  style.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      ...
      --font:'Outfit',sans-serif; --mono:'DM Mono',monospace; --display:'Syne',sans-serif;
    }
    body { background:var(--bg); color:var(--text); font-family:var(--font); }
    .pa-layout { display:flex; min-height:100vh; }
    .pa-sidebar { width:220px; min-height:100vh; background:var(--surface); border-right:1px solid var(--border); display:flex; flex-direction:column; position:fixed; left:0; top:0; z-index:100; padding-bottom:24px; }
    ...
    .pa-logo-text { font-family:var(--display); font-weight:800; font-size:20px; letter-spacing:1px; }
    .pa-nav { flex:1; padding:16px 10px; display:flex; flex-direction:column; gap:2px; overflow-y:auto; }
    .pa-nav-section { font-size:10px; font-weight:600; letter-spacing:1.4px; color:var(--muted); text-transform:uppercase; padding:10px 10px 4px; }
    .pa-nav-item { display:flex; align-items:center; gap:10px; padding:9px 10px; border-radius:8px; cursor:pointer; font-size:13.5px; font-weight:400; color:var(--muted2); transition:all 0.18s; border:1px solid transparent; }
    .pa-nav-item:hover { background:var(--surface2); color:var(--text); }
    .pa-nav-item.active { background:rgba(79,142,255,0.12); color:var(--accent); border-color:rgba(79,142,255,0.2); font-weight:500; }
    ...
    .pa-avatar { width:32px; height:32px; border-radius:50%; background:linear-gradient(135deg,var(--accent),var(--accent2)); display:flex; align-items:center; justify-content:center; font-size:12px; font-weight:700; color:white; flex-shrink:0; }
    .pa-user-name { font-size:13px; font-weight:500; }
    .pa-user-role { font-size:11px; color:var(--accent2); font-family:var(--mono); }
    .pa-logout { display:flex; align-items:center; gap:8px; padding:8px 10px; border-radius:8px; cursor:pointer; font-size:13px; color:var(--danger); transition:background 0.18s; border:none; background:transparent; width:100%; font-family:var(--font); }
    .pa-logout:hover { background:rgba(255,77,109,0.1); }
    .pa-main { margin-left:220px; flex:1; min-height:100vh; }
    .pa-topbar { padding:16px 28px; display:flex; align-items:center; justify-content:space-between; border-bottom:1px solid var(--border); position:sticky; top:0; background:rgba(8,11,18,0.88); backdrop-filter:blur(14px); z-index:50; }
    .pa-page-title { font-family:var(--display); font-size:22px; font-weight:700; }
    .pa-topbar-right { display:flex; align-items:center; gap:10px; }
    .pa-notif-btn { position:relative; width:36px; height:36px; background:var(--surface2); border:1px solid var(--border2); border-radius:8px; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:16px; transition:all 0.18s; color:var(--text); }
    .pa-notif-btn:hover { border-color:var(--accent); }
    ...
    .pa-api-pill { display:flex; align-items:center; gap:7px; background:var(--surface2); border:1px solid var(--border2); padding:7px 13px; border-radius:8px; font-size:12.5px; }
    .pa-api-dot { width:7px; height:7px; border-radius:50%; background:var(--accent2); animation:blink 2s infinite; }
    @keyframes blink { 0%,100%{opacity:1} 50%{opacity:0.3} }
    .pa-api-val { font-family:var(--mono); font-weight:500; color:var(--accent2); }
    .pa-content { padding:28px; }
    .pa-section-hd { font-family:var(--display); font-size:18px; font-weight:700; margin-bottom:18px; display:flex; align-items:center; gap:10px; color:var(--text); }
    .pa-section-hd::after { content:''; flex:1; height:1px; background:var(--border); }
    .pa-filter-row { display:flex; gap:8px; margin-bottom:22px; }
    .pa-filter-btn { padding:7px 16px; border-radius:6px; font-size:12.5px; font-weight:500; cursor:pointer; border:1px solid var(--border2); background:var(--surface2); color:var(--muted2); transition:all 0.18s; font-family:var(--font); }
    .pa-filter-btn:hover,.pa-filter-btn.active { background:rgba(79,142,255,0.12); color:var(--accent); border-color:rgba(79,142,255,0.3); }
    .pa-stats { display:grid; grid-template-columns:repeat(4,1fr); gap:14px; margin-bottom:24px; }
    .pa-stat { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:18px 20px; position:relative; overflow:hidden; transition:border-color 0.2s; }
    .pa-stat:hover { border-color:var(--border2); }
    .pa-stat::before { content:''; position:absolute; top:0; left:0; right:0; height:2px; }
    ...
    .pa-stat-label { font-size:11.5px; color:var(--muted2); font-weight:500; margin-bottom:10px; }
    .pa-stat-val { font-family:var(--display); font-size:32px; font-weight:800; line-height:1; margin-bottom:6px; }
    .pa-stat-sub { font-size:11.5px; color:var(--muted); }
    ...
    .pa-chart-card { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:20px; }
    .pa-chart-title { font-family:var(--display); font-size:14px; font-weight:700; margin-bottom:18px; }
    .custom-tooltip { background:var(--surface3); border:1px solid var(--border2); border-radius:8px; padding:10px 14px; font-size:12px; }
    .pa-quick { display:flex; gap:10px; margin-bottom:24px; flex-wrap:wrap; }
    .pa-quick-btn { display:flex; align-items:center; gap:7px; padding:9px 16px; border-radius:8px; font-size:13px; font-weight:500; cursor:pointer; border:1px solid var(--border2); background:var(--surface2); color:var(--text); transition:all 0.2s; font-family:var(--font); }
    ...
    .pa-sys-pill { display:flex; align-items:center; gap:7px; background:var(--surface); border:1px solid var(--border); padding:8px 14px; border-radius:8px; font-size:12.5px; color:var(--muted2); }
    ...
    .pa-activity { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:20px; margin-top:20px; }
    .pa-activity-item { display:flex; align-items:center; gap:12px; padding:9px 0; border-bottom:1px solid var(--border); }
    ...
    .pa-activity-text { flex:1; font-size:13px; }
    .pa-activity-time { font-size:11px; color:var(--muted); font-family:var(--mono); }
    .pa-table-wrap { background:var(--surface); border:1px solid var(--border); border-radius:12px; overflow:hidden; }
    .pa-table-toolbar { padding:14px 18px; display:flex; align-items:center; gap:10px; border-bottom:1px solid var(--border); flex-wrap:wrap; }
    .pa-search { flex:1; min-width:180px; background:var(--surface2); border:1px solid var(--border2); border-radius:8px; padding:8px 14px; font-size:13px; color:var(--text); font-family:var(--font); outline:none; transition:border-color 0.18s; }
    .pa-search:focus { border-color:var(--accent); }
    .pa-search::placeholder { color:var(--muted); }
    .pa-btn { padding:8px 16px; border-radius:8px; font-size:13px; font-weight:500; cursor:pointer; border:1px solid var(--border2); background:var(--surface2); color:var(--text); transition:all 0.18s; font-family:var(--font); display:flex; align-items:center; gap:6px; }
    .pa-btn:hover { background:var(--surface3); border-color:var(--accent); color:var(--accent); }
    .pa-btn.primary { background:var(--accent); border-color:var(--accent); color:white; }
    .pa-btn.primary:hover { background:#6fa8ff; border-color:#6fa8ff; color:white; }
    ...
    th { padding:12px 18px; text-align:left; font-size:11px; font-weight:600; letter-spacing:0.8px; text-transform:uppercase; color:var(--muted); }
    ...
    .pa-badge { padding:3px 9px; border-radius:20px; font-size:11px; font-weight:600; font-family:var(--mono); display:inline-block; }
    .pa-badge.active{background:rgba(0,229,176,0.12);color:var(--accent2)} .pa-badge.blocked{background:rgba(255,77,109,0.12);color:var(--danger)}
    .pa-badge.success{background:rgba(0,229,176,0.12);color:var(--accent2)} .pa-badge.failed{background:rgba(255,77,109,0.12);color:var(--danger)}
    .pa-badge.free{background:rgba(90,106,138,0.2);color:var(--muted2)} .pa-badge.starter{background:rgba(79,142,255,0.12);color:var(--accent)}
    .pa-badge.growth{background:rgba(255,181,71,0.12);color:var(--warn)} .pa-badge.business{background:rgba(167,139,250,0.12);color:#a78bfa}
    .pa-plans-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin-bottom:24px; }
    .pa-plan-card { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:22px; transition:all 0.2s; position:relative; }
    .pa-plan-card:hover { border-color:var(--border2); }
    .pa-plan-card.featured { border-color:rgba(79,142,255,0.35); }
    .pa-plan-name { font-family:var(--display); font-size:18px; font-weight:800; margin-bottom:4px; }
    .pa-plan-price { font-family:var(--mono); font-size:28px; font-weight:500; color:var(--accent); margin-bottom:14px; }
    .pa-plan-price span { font-size:14px; color:var(--muted); }
    .pa-plan-feat { list-style:none; margin-bottom:16px; display:flex; flex-direction:column; gap:7px; }
    .pa-plan-feat li { font-size:13px; color:var(--muted2); display:flex; align-items:center; gap:7px; }
    .pa-plan-feat li::before { content:'✓'; color:var(--accent2); font-weight:700; }
    .pa-plan-users { font-size:11px; color:var(--muted); font-family:var(--mono); margin-bottom:12px; }
    .pa-popular-tag { position:absolute; top:-1px; right:16px; background:var(--accent); color:white; font-size:10px; font-weight:700; padding:3px 10px; border-radius:0 0 8px 8px; font-family:var(--mono); }
    .pa-plan-form { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:22px; margin-bottom:24px; }
    .pa-form-title { font-family:var(--display); font-size:16px; font-weight:700; margin-bottom:16px; }
    .pa-form-grid { display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; margin-bottom:12px; }
    .pa-input { background:var(--surface2); border:1px solid var(--border2); border-radius:8px; padding:9px 14px; font-size:13px; color:var(--text); font-family:var(--font); outline:none; width:100%; transition:border-color 0.18s; }
    .pa-input:focus { border-color:var(--accent); }
    .pa-input::placeholder { color:var(--muted); }
    .pa-input-label { font-size:11px; font-weight:600; letter-spacing:0.8px; text-transform:uppercase; color:var(--muted); margin-bottom:5px; }
    .pa-credits-grid { display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:24px; }
    .pa-credit-card { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:22px; }
    .pa-credit-title { font-family:var(--display); font-size:16px; font-weight:700; margin-bottom:16px; }
    ...
    .pa-credit-stats { display:flex; justify-content:space-between; font-size:12.5px; color:var(--muted2); }
    .pa-credit-big { font-family:var(--display); font-size:40px; font-weight:800; color:var(--accent2); line-height:1; margin-bottom:4px; }
    .pa-notif-list { display:flex; flex-direction:column; gap:8px; }
    .pa-notif-item { display:flex; align-items:flex-start; gap:12px; background:var(--surface); border:1px solid var(--border); border-radius:10px; padding:14px 16px; transition:all 0.18s; }
    .pa-notif-item.unread { border-left:3px solid var(--accent); }
    .pa-notif-item:hover { background:var(--surface2); }
    .pa-notif-icon { font-size:20px; flex-shrink:0; margin-top:1px; }
    .pa-notif-content { flex:1; }
    .pa-notif-msg { font-size:13.5px; margin-bottom:3px; }
    .pa-notif-time { font-size:11px; color:var(--muted); font-family:var(--mono); }
    .pa-unread-dot { width:8px; height:8px; border-radius:50%; background:var(--accent); flex-shrink:0; margin-top:6px; }
    .pa-modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.75); backdrop-filter:blur(8px); z-index:200; display:flex; align-items:center; justify-content:center; }
    .pa-modal { background:var(--surface); border:1px solid var(--border2); border-radius:14px; padding:28px; width:480px; max-width:92vw; position:relative; animation:modalIn 0.28s cubic-bezier(0.34,1.56,0.64,1); max-height:90vh; overflow-y:auto; }
    @keyframes modalIn { from{transform:scale(0.92);opacity:0} }
    .pa-modal-title { font-family:var(--display); font-size:20px; font-weight:800; margin-bottom:6px; }
    .pa-modal-sub { font-size:13px; color:var(--muted2); margin-bottom:22px; }
    .pa-modal-close { position:absolute; top:14px; right:14px; width:30px; height:30px; border-radius:7px; background:var(--surface2); border:none; cursor:pointer; color:var(--muted2); font-size:16px; display:flex; align-items:center; justify-content:center; transition:all 0.18s; }
    .pa-modal-close:hover { background:var(--surface3); color:var(--text); }
    .pa-form-row { display:flex; flex-direction:column; gap:12px; }
    .pa-form-field { display:flex; flex-direction:column; gap:5px; }
    .pa-settings-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:14px; }
    .pa-settings-card { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:20px; transition:all 0.2s; }
    .pa-settings-card:hover { border-color:var(--border2); }
    .pa-settings-icon { font-size:24px; margin-bottom:10px; }
    .pa-settings-title { font-family:var(--display); font-size:15px; font-weight:700; margin-bottom:5px; }
    .pa-settings-desc { font-size:12.5px; color:var(--muted2); margin-bottom:14px; line-height:1.5; }
    .pa-toast { position:fixed; bottom:24px; right:24px; background:var(--surface); border:1px solid var(--accent2); border-radius:10px; padding:13px 18px; display:flex; align-items:center; gap:10px; font-size:13.5px; z-index:999; transform:translateY(60px); opacity:0; transition:all 0.32s cubic-bezier(0.34,1.56,0.64,1); max-width:320px; box-shadow:0 8px 32px rgba(0,0,0,0.4); }
    .pa-toast.show { transform:translateY(0); opacity:1; }
    ::-webkit-scrollbar{width:4px} ::-webkit-scrollbar-track{background:transparent} ::-webkit-scrollbar-thumb{background:var(--border2);border-radius:2px}
    .pa-select { background:var(--surface2); border:1px solid var(--border2); border-radius:8px; padding:8px 12px; font-size:13px; color:var(--text); font-family:var(--font); outline:none; cursor:pointer; }
    .pa-select:focus { border-color:var(--accent); }
    .pa-textarea { background:var(--surface2); border:1px solid var(--border2); border-radius:8px; padding:12px 14px; font-size:13.5px; color:var(--text); font-family:var(--font); outline:none; width:100%; resize:none; height:100px; transition:border-color 0.18s; }
    ...
    .pa-topup-user { display:flex; align-items:center; gap:10px; padding:10px 12px; border-radius:8px; border:1px solid var(--border); background:var(--surface2); cursor:pointer; transition:all 0.18s; }
    .pa-topup-user:hover,.pa-topup-user.selected { border-color:var(--accent); background:rgba(79,142,255,0.07); }
    .pa-topup-name { font-size:13px; font-weight:500; flex:1; }
    ...
    .pa-gen-card { background:var(--surface); border:1px solid var(--border); border-radius:12px; padding:22px; }
    .pa-preset-row { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:16px; }
    .pa-preset { padding:6px 12px; border-radius:6px; font-size:12px; font-weight:500; cursor:pointer; border:1px solid var(--border2); background:var(--surface2); color:var(--muted2); transition:all 0.18s; font-family:var(--font); }
    .pa-preset:hover,.pa-preset.active { border-color:var(--accent); color:var(--accent); background:rgba(79,142,255,0.1); }
    .pa-upload-label { display:flex; align-items:center; justify-content:center; gap:8px; border:2px dashed var(--border2); border-radius:10px; padding:22px; cursor:pointer; font-size:14px; color:var(--muted2); transition:all 0.2s; margin-bottom:14px; }
    ...
    .pa-angle-btn { padding:8px; border-radius:6px; font-size:12px; cursor:pointer; border:1px solid var(--border2); background:var(--surface2); color:var(--muted2); transition:all 0.18s; font-family:var(--font); text-align:center; }
    ...
    .pa-gen-shimmer-text { position:absolute; bottom:16px; left:0; right:0; text-align:center; font-size:13px; color:var(--accent); font-family:var(--mono); animation:pulse 1.5s ease infinite; }
    ...
    .pa-history-card { background:var(--surface); border:1px solid var(--border); border-radius:12px; overflow:hidden; transition:all 0.2s; cursor:pointer; }
    .pa-history-card:hover { border-color:var(--border2); transform:translateY(-2px); box-shadow:0 8px 24px rgba(0,0,0,0.3); }
    ...
    .pa-history-user { font-size:13.5px; font-weight:600; margin-bottom:6px; }
    .pa-history-meta { display:flex; flex-direction:column; gap:4px; }
    .pa-history-row { display:flex; gap:6px; align-items:center; font-size:12px; color:var(--muted2); }
    .pa-history-row b { color:var(--text); }
    .pa-history-footer { padding:10px 14px; border-top:1px solid var(--border); display:flex; align-items:center; justify-content:space-between; font-size:11px; }
    .pa-history-detail { display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:18px; }
    .pa-detail-field { background:var(--surface2); border:1px solid var(--border); border-radius:8px; padding:10px 14px; }
    .pa-detail-label { font-size:10px; font-weight:600; letter-spacing:0.8px; text-transform:uppercase; color:var(--muted); margin-bottom:4px; }
    .pa-detail-val { font-size:13.5px; font-weight:500; font-family:var(--mono); }
    .pa-detail-full { grid-column:1/-1; background:var(--surface2); border:1px solid var(--border); border-radius:8px; padding:10px 14px; }
...
export default function PostifyAdmin() {
  ...
  // ── USER DASHBOARD STUDIO COPIED STATE ──
  ...
  const handleGenerateStudio = () => {
    ...
    setTimeout(() => {
      ...
      setGenHistory(prev => [{
        ...
        date: date,
        time: time,
        preset: activeStyle, background: background,
  ...
  // ── LOGOUT: clears localStorage → goes to your original /login page
  const handleLogout = () => {
  ...
  const handleToggleUser  = (id) => { 
    const updated = users.map(u => u.id === id ? { ...u, status: u.status === "Active" ? "Blocked" : "Active" } : u);
    setUsers(updated); 
  ...
  const handleAddUser     = () => { 
    if(!newUser.name||!newUser.email){showToast("Fill all fields!");return;} 
    const updated = [...users,{...newUser,id:Date.now(),images:0,joined:new Date().toISOString().split("T")[0]}];
    setUsers(updated); 
    localStorage.setItem('postify_users', JSON.stringify(updated));
    setNewUser({name:"",email:"",plan:"Free",credits:20}); 
    setModal(null); 
    showToast("User added!"); 
  };
  
  const handleTopup       = () => { 
    if(!topupUser||!topupAmt)return; 
    const updated = users.map(u=>u.id===topupUser?{...u,credits:u.credits+Number(topupAmt)}:u);
    setUsers(updated); 
    localStorage.setItem('postify_users', JSON.stringify(updated));
    setModal(null); setTopupUser(null); setTopupAmt(""); 
    showToast("Credits added!"); 
  };
  
  const handleAddPlan     = () => { 
    if(!newPlan.name||!newPlan.price||!newPlan.credits){showToast("Fill all fields!");return;} 
    const updated = [...plans,newPlan];
    setPlans(updated); 
    localStorage.setItem('postify_plans', JSON.stringify(updated));
    setNewPlan({name:"",price:"",credits:"",features:""}); 
    showToast("Plan added!"); 
  };
  
  const handleSaveEditPlan= () => { 
    const updated = plans.map((p,i)=>i===editingPlan.index?editingPlan.data:p);
    setPlans(updated); 
    localStorage.setItem('postify_plans', JSON.stringify(updated));
    setModal(null); 
  ...
  const markAllRead       = () => { setNotifs(notifs.map(n=>({...n,read:true}))); showToast("All read!"); };

  // Listen for cross-tab updates (like when a user purchases a plan in another tab)
  useEffect(() => {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      ...
      setGenHistory(prev => [{
        ...
        preset: selectedPreset||"Custom", background: bg,
        description: desc||"(No description)", angle,
        creditsUsed: 8, status: "Success", thumbnail: preview,
      }, ...prev]);
        ...
        <div className="pa-topbar">
          ...
          <div className="pa-topbar-right">
            <div className="pa-api-pill">
              ...
              <span style={{color:"var(--muted2)",fontSize:12}}>API Credits</span>
        ...
        <div className="pa-content">
                        ...
                        <div className="pa-history-meta">
                          ...
                          <div className="pa-history-row">📐 Angle: <b>{g.angle}</b></div>
                            <div className="pa-history-row" style={{flexDirection:"column",alignItems:"flex-start",gap:2}}>
                              ...
                              <span style={{color:"var(--muted2)",fontSize:12,lineHeight:1.3}}>
                                {g.description && g.description !== "(No description)" ? g.description : "-"}
                              </span>
                            </div>
                        </div>
                      </div>
          ...
          {view==="users"&&(
            <div className="pa-table-wrap">
              <div className="pa-table-toolbar">
                ...
                <button className="pa-btn" onClick={()=>setModal("topup")}>⬡ Top-up</button>
              </div>
              <table>
                  ...
                  {filteredUsers.map(u=>(
                    <tr key={u.id} onClick={()=>{setModalData(u);setModal("userDetail");}}>
                      ...
                      <td onClick={e=>e.stopPropagation()}>
                        <button className={`pa-btn ${u.status==="Active"?"danger":"success"}`} style={{fontSize:12,padding:"5px 12px"}} onClick={()=>handleToggleUser(u.id)}>
          ...
          {view==="transactions"&&(
            <div className="pa-table-wrap">
              ...
              <table>
                <thead><tr><th>TXN ID</th><th>User</th><th>Plan</th><th>Amount</th><th>Credits</th><th>Method</th><th>Date</th><th>Status</th></tr></thead>
                <tbody>
                  {filteredTx.filter(t=>!search||t.user.toLowerCase().includes(search.toLowerCase())).map(t=>(
                    <tr key={t.id}>
                      ...
                      <td style={{fontSize:12,color:"var(--muted2)"}}>{t.paymentMethod}</td>
          ...
          {view==="credits"&&(
            <div className="pa-credits-grid">
              ...
              <div className="pa-credit-card">
                ...
                <button className="pa-btn" style={{width:"100%",justifyContent:"center",marginTop:8}} onClick={()=>setModal("topup")}>+ Manual Top-up</button>
              </div>
            </div>
          )}
            ...
            <>
              ...
              <div className="pa-notif-list">
                {notifs.map(n=>{
                  ...
                  return(
                    <div key={n.id} className={`pa-notif-item ${!n.read?"unread":""}`} onClick={()=>setNotifs(notifs.map(x=>x.id===n.id?{...x,read:true}:x))}>
      ...
      {modal==="topup"&&(
        <div className="pa-modal-overlay" onClick={()=>setModal(null)}>
          <div className="pa-modal" onClick={e=>e.stopPropagation()}>
            ...
            <button className="pa-btn primary" style={{width:"100%",justifyContent:"center"}} onClick={handleTopup}>⬡ Add Credits</button>
          </div>
        </div>
      )}
          ...
          <div className="pa-modal" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
              {[["Plan",modalData.plan],["Credits",modalData.credits],["Images",modalData.images],["Joined",modalData.joined],["Status",modalData.status]].map(([k,v])=>(
                <div key={k} style={{background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:8,padding:"10px 14px"}}>
                  <div style={{fontSize:11,color:"var(--muted)",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.8px"}}>{k}</div>
                  <div style={{fontFamily:"var(--mono)",fontSize:14,fontWeight:500}}>{v}</div>
                </div>
              ))}
            </div>
            <div style={{marginBottom:18}}>
              <div style={{fontSize:12,fontWeight:600,color:"var(--muted)",textTransform:"uppercase",letterSpacing:"0.8px",marginBottom:8}}>
              ...
              {genHistory.filter(g=>g.user===modalData.name).length===0
                ...
                :genHistory.filter(g=>g.user===modalData.name).slice(0,3).map((g,i)=>(
                  <div key={i} style={{background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:8,padding:"8px 12px",fontSize:12,marginBottom:6}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button className="pa-btn" style={{flex:1,justifyContent:"center"}} onClick={()=>{setModal("topup");setTopupUser(modalData.id);}}>⬡ Top-up</button>
